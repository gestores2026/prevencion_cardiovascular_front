      ...
      <div class="content">
        ...
        <div class="stats-grid">
          ...
          <div class="stat-card stat-green">
            ...
            <div class="stat-value">{{ stats.promedioRiesgo }}%</div>
          </div>
        </div>

        <div class="stats-grid" style="margin-top: 0;">
          <div class="stat-card">
            <div class="stat-label">Tests este mes</div>
            <div class="stat-value">{{ stats.totalTestsEsteMes }}</div>
          </div>
          <div class="stat-card stat-red">
            <div class="stat-label">Fumadores</div>
            <div class="stat-value">{{ stats.pacientesFumadores }}</div>
          </div>
          <div class="stat-card stat-green">
            <div class="stat-label">Riesgo bajo</div>
            <div class="stat-value">{{ stats.pacientesRiesgoBajo }}</div>
          </div>
          <div class="stat-card">
            <div class="stat-label">Alertas activas</div>
            <div class="stat-value stat-red-val">{{ totalAlertas }}</div>
          </div>
        </div>
        ...
        <div class="grid-2">
          ...
          <div class="card">
            <div class="card-header">
              <span class="card-title">Mis pacientes recientes</span>
        ...
        <div class="card" style="margin-top: 16px;">
          ...
          <div class="alerta-list">
            <AlertaItem
              ...
              :alerta="a"
            />
            <div v-if="alertas.length === 0" class="empty">Sin alertas activas</div>
          </div>
        </div>
...
import { obtenerEstadisticasMedico } from '../../api/estadisticasApi.js'
import { listarMisPacientes } from '../../api/usuarioApi.js'
import { listarAlertas, contarAlertas } from '../../api/alertaApi.js'
...
const stats = ref({
  ...
  pacientesRiesgoModerado: 0,
  pacientesRiesgoBajo: 0,
  promedioRiesgo: 0,
  pacientesFumadores: 0,
  totalTestsEsteMes: 0
})
const pacientes = ref([])
const alertas = ref([])
...
onMounted(async () => {
  try {
    const [resStats, resPacientes, resAlertas, resCount] = await Promise.all([
      obtenerEstadisticasMedico(auth.usuario.id),
      listarMisPacientes(auth.usuario.id),
      listarAlertas(),
      contarAlertas()
    ])
    ...
    totalAlertas.value = resCount.data.total

    if (stats.value.totalPacientes > 0) {
      pieData.value = {
